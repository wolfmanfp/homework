function createRandomTable(cols, rows){
			...
			function doexcel($table){
				...
				$table.find('tr:not(:first) td:not(.head)').each(function(){
						var value = $(this).html();
                        var input = $('<input type="text">').val(value).change(function(){
						// a függvényen belül a this az input elem
						var value = parseInt( $(this).val() ); // kivesszük az értéket
						...
						$(this).val(value); // visszaírjuk az input-ba a módosított értéket
					});
						$(this).append(input);
					}
				);
                ...
                // létrehozzuk a grafikon divet és beszúrjuk a dom-ba
					...
					$table.find('td.head').click(function(){
						...
						// lekérdezzük a táblázat elemet, ami a cellának a 3. szülője
						// a tbody elemet kézzel nem hoztuk létre, azt a böngésző állítja elő
						$table = $(this).
							parent(). // tr
							parent(). // tbody
							parent(); // table
